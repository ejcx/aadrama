...
const TopByMapPage = () => {
  const [topPlayersByMap, setTopPlayersByMap] = useState<TopPlayer[]>([]);
  const [maps, setMaps] = useState<MapInfo[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedMap, setSelectedMap] = useState<string>("Urban Assault");
    ...
    const fetchTopPlayersByMap = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE}/analytics/top-players/map/${encodeURIComponent(selectedMap)}?${params.toString()}`
        );
        const data = await response.json();

        if (Array.isArray(data)) {
          setTopPlayersByMap(data);
        } else if (data.players && Array.isArray(data.players)) {
          setTopPlayersByMap(data.players);
        } else {
          setTopPlayersByMap([]);
        }
        setError(null);
  ...
  }, [selectedMap, sortBy, limit, timeRange, startTime, endTime]);
