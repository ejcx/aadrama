-- Map reroll voting for tiered (random map) scrims.
-- Same threshold as team reroll: (total_players / 2) + 1
-- Only applies when map_choice = 'tiered' and a map has already been assigned.

ALTER TABLE public.scrim_players
ADD COLUMN IF NOT EXISTS voted_map_reroll BOOLEAN NOT NULL DEFAULT FALSE;

-- Weighted pick from the tiered pool, optionally excluding a map (for rerolls).
-- Weights mirror assign_tiered_map_if_needed / lib/scrim/tiered-maps.ts.
...
BEGIN
  ...
  IF v_picked IS NULL THEN
    -- Empty exclude pool or float edge: fall back without exclude, then Insurgent Camp
...
-- Force a new map for a tiered scrim that already has one assigned.
